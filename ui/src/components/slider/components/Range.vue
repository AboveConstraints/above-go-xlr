...
export default {
  ...
  props: {
    ...
    backgroundColour: {type: String, required: false, default: 'var(--ag-surface)'},
...
.slider {
  background: linear-gradient(
    ...
    v-bind(colour) 0%,
    v-bind(colour) 50%,
    v-bind(backgroundColour) 50%,
    v-bind(backgroundColour) 100%
  );
  ...
  /* Glow under the filled portion — subtle, branded */
  filter: drop-shadow(0 0 3px color-mix(in srgb, v-bind(colour) 60%, transparent));
}
...
input[type='range']::-webkit-slider-thumb {
  ...
  height: 22px;
  border-radius: 3px;
  background: linear-gradient(180deg, #ffffff 0%, v-bind(colour) 100%);
  border: none;
  box-shadow:
    0 0 0 1.5px rgba(0,0,0,0.6),
    0 0 10px color-mix(in srgb, v-bind(colour) 70%, transparent),
...
input[type='range']::-webkit-slider-thumb:active {
  ...
  box-shadow:
    0 0 0 1.5px rgba(0,0,0,0.6),
    0 0 16px color-mix(in srgb, v-bind(colour) 90%, transparent),
