<template>
  <ExpandoGroupContainer :title="$t('message.microphone.gate.title')" @expando-clicked="toggleAdvanced" :expanded="isAdvanced()" variant="toggle">
      ...
      <div v-if="showWaveForm" class="meter-wrap">
        <AudioMeter ref="audioMeter" :active="showWaveForm" :fade_below="getAdjustedThreshold()" :show_estimated=true />
        <button class="meter-close" @click="closeWaveForm" :title="$t('message.modalButtons.close')">
...
export default {
  ...
  methods: {
    ...
    toggleAdvanced() {
      ...
      store.getActiveDevice().settings.display.gate = mode;
