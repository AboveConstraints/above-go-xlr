...
export default {
  ...
  methods: {
    ...
    draw: function (timestamp) {
      ...
      // Always clear and draw the static background zones
      this.canvas.clearRect(0, 0, this.canvas_size.width, this.canvas_size.height)
      ...
      // Only draw the waveform line when there are data points
      ...
      // Always draw the gate threshold line on top
    ...
    render_text: function(text, x, y, color = "var(--ag-text)", text_align = "left", text_baseline = "middle") {
    ...
    draw_fade_below: function() {
      ...
      this.canvas.strokeStyle = 'var(--ag-text)';
      this.canvas.beginPath();
