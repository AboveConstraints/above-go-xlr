<template>
  <ExpandoGroupContainer :title="$t('message.microphone.equaliser.title')" @expando-clicked="toggleAdvanced()" :expanded="isAdvanced()" variant="toggle">
    <template #right>
...
export default {
  ...
  methods: {
    ...
    toggleAdvanced() {
      ...
      store.getActiveDevice().settings.display.equaliser = mode;
    ...
    getBackgroundColour(index) {
      if (isDeviceMini()) {
        if (index < 3) {
          return "var(--ag-surface)";
        }

        if (index < 5) {
          return "var(--ag-surface-2)";
        }

        return "var(--ag-divider)";
      }
