<template>
  <div id="main">
    ...
    <template v-if="isDeviceSet()">
      <div class="app-body">
        <!-- ===== Profiles sidebar (icon rail, hover to expand) — hosts brand, profiles, system & mixer toggle ===== -->
        <ProfileSidebar />

        <!-- ===== Main content ===== -->
        <main class="content">
          <h1 class="sr-only">{{ $t('message.navigation.accessibilityDeviceSection') }}</h1>
          <div aria-hidden="true" class="visualiser">
            <GoXLRVisualiser :highlighted-areas="this.visualiserHighlightAreas" @area-click="onAreaClicked"/>
          </div>
          <Tabs ref="device-tabs" variant="pill" centered @on-change="onTabChange"
                :label="$t('message.navigation.accessibilityDeviceSection')">
            <Tab id="mixer" :name="$t('message.navigation.mixer')">
              <ContentContainer>
                <Mixer/>
              </ContentContainer>
            </Tab>
            <Tab id="mic" :name="$t('message.navigation.microphone')" selected>
              <Mic/>
            </Tab>
...
import ProfileSidebar from "@/components/sections/files/ProfileSidebar.vue";
...
export default {
  ...
  methods: {
    ...
    loadSystemTab() {
      // System now lives in the sidebar (opened as a modal), not a device tab,
      // so there's no tab to navigate to here.
    },
